...
class ChangeTrackingDeploymentQueries:
    ...
    def entity_search(
        ...
        deployment_search_field = Field(
            name="deploymentSearch", arguments=filter, subfields=[results_field]
        )

        entity_field = Field(
            name="entity", arguments={"guid": guid}, subfields=[deployment_search_field]
        )

        actor_field = Field(name="actor", subfields=[entity_field])
    ...
    def create(deployment_event, append_to_query: GraphQLQuery = None):
        ...
        event_dict = dict(
            entityGuid=deployment_event.entity_guid, version=deployment_event.version
        )
        ...
        add_to_dict_if_not_none(
            event_dict, "deploymentType", deployment_event.deployment_type
        )
        ...
        create_field = Field(
            ...
            subfields=["deploymentId", "entityGuid"],
        )

        mutation_field = Field(name="mutation", subfields=[create_field])
