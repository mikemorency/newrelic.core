...
class TestNrModule(ModuleTestCase):
    def __prepare(self, mocker):
        ...
        self.default_args = dict(
            ...
            data_aggregation_timer=5,
            critical_incident=dict(failed_locations_threshold=1, duration=2),
            warning_incident=dict(failed_locations_threshold=1, duration=2),
        )
        ...
        for k, v in self.default_args.items():
            if k.endswith("incident"):
                self.test_condition.incident_terms.append(
                    IncidentTerm(
                        threshold=v["failed_locations_threshold"],
                        operator="ABOVE_OR_EQUALS",
                        duration=v["duration"],
                        occurrences="AT_LEAST_ONCE",
                        priority=k.split("_", maxsplit=1)[0].upper(),
                    )
                )
            else:
                setattr(self.test_condition, k, v)
        self.test_condition.nrql_query = (
            "SELECT sum(constant) FROM ("
            "SELECT if(latest(result)='FAILED', 1, 0) FROM SyntheticCheck "
            "WHERE entityGuid = '1' FACET locationLabel)"
        )
